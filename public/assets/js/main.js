...
(function(){
  ...
  const countNumbers = [...document.querySelectorAll('[data-count-to]')];
  if(countNumbers.length){
    const animateCount = (number) => {
      ...
      const tick = (now) => {
        ...
        number.textContent = `${Math.round(target * eased).toLocaleString('en-GB')}${suffix}`;
    ...
    if('IntersectionObserver' in window){
      ...
      countNumbers.forEach(number => observer.observe(number));
    }else{
      countNumbers.forEach(animateCount);
    }
  }
